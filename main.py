    ...
    if cached:
        ...
        if uploaded_file:
            ...
            with st.spinner("Extracting and caching textbook..."):
                ...
                cur = conn.cursor()
                cur.execute(
                    "INSERT INTO textbook_cache (subject, content, filename) VALUES (%s, %s, %s) ON CONFLICT (subject) DO UPDATE SET content = EXCLUDED.content, filename = EXCLUDED.filename, timestamp = CURRENT_TIMESTAMP",
...
def get_ai_response(prompt, context, subject_name, allow_external=False):
    try:
        ...
        # We avoid using Postgres-specific similarity functions since some databases may not have them.
        prev_match = None
        context_limit = 40000
        try:
            # Fetch all previous queries and responses for this subject
            conn = get_db_connection()
            cur = conn.cursor()
            ...
            for row in rows:
                try:
                    # row is a tuple like (query, response)
                    q = row[0]
                    ...
                    continue
            # Consider it a match only if the similarity is reasonably high
            if best_score > 0.4 and best_query is not None:
                prev_match = {'query': best_query, 'response': best_response}
        except Exception:
            # If any error occurs (e.g. database unavailable), skip matching
            prev_match = None
        prev_context = ""
        if prev_match:
        ...
        if search_terms:
            ...
            if not relevant_snippets:
                # If no specific relevant snippets were found, take a head and tail
                # slice of the full context to stay within the token limit.
                context_snippet = (
                    context[:20000] + "\n... [SNIP] ...\n" + context[-10000:]
                )[:context_limit]
            else:
                # Build the context snippet by joining unique relevant snippets. We
                # use set() to deduplicate and then join with a clear delimiter
                # between sections.
